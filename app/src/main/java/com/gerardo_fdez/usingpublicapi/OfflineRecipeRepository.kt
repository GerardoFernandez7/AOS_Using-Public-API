...
data class OfflineRecipeRepository(
    private val dao: Any,
    ...
    fun getRecipe(idMeal: String):  Flow<List<MealRecipe>> = flow{
        if (InternetCheck(context)) {
            ...
            val rFROMAPI = apiService.getRecipes(idMeal).meals
            Log.i("API Response", "Número de recetas obtenidas de la API: ${rFROMAPI?.size ?: 0}")


            // Mapea la lista a una nueva lista de 'Meal' con la categoría establecida
            val rWithM = rFROMAPI?.map { recipe ->
                MealRecipe(
                    idMeal = idMeal,
            ...
            emit(rWithM ?: emptyList())
        } else {
            Log.i("No hay internet", "no hay internet")
            // Si no hay internet, obtener de la base de datos local
            recipeDao.getAllRecipe(idMeal).collect { rFROMAPI ->
                Log.i("Database", "Recetas recuperadas de la base de datos local: ${rFROMAPI.size}")
                emit(rFROMAPI)
            }
        }
    }
